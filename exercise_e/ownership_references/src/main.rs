...
fn inspect(i_string: &String) {
    if i_string.ends_with("s") {
        println!("{i_string} is plural");
    } else {
        println!("{i_string} is singular");
    }
}

fn change(c_string: &mut String) -> &mut String {
    if c_string.ends_with("s") {
        return c_string;
    }
...
fn eat(e_string: String) -> bool {
    e_string.starts_with("b") && e_string.contains("a")
}

fn bedazzle(b_string: &mut String) -> () {
    *b_string = "sparkly".to_string();
}
